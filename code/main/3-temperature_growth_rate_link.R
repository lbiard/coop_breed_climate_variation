...
##          sample                                                        ##
##                                                                        ##
##  This file estimate the effect of temperature on population growth     ##
...
samples_to_remove <- unique(bird_data$ID_block[is.na(bird_data$delta_temperature)])
...
for (i in 1:length(list_of_samples)) {
  ...
  model_spline_ar <- quiet_brm(
    pop_growth_rate ~ delta_temperature +
      s(year, bs="tp", k=5) +
      ar(time=year)
    ,
    ...
    prior = c(
      ...
      set_prior("normal(0,1)", class = "b", coef = "delta_temperature"),
  ...
  store_posterior_effect_size[[i]] <- as.data.frame(
    cbind(as.numeric(post_model_spline$b_delta_temperature),
...
getwd()
#write.table(data_for_meta_regression, "population_sensitivity_temperature.txt")
...
figure_s2 <- ggplot(result_summary_sorted, aes(y = rank)) +   
  ...
  geom_point(aes(x = mean), size = 0.04) +
    ...
    xlab(expression(paste("Effect size of temperature change on log ", lambda, " (per degree C)")))+
    coord_cartesian(xlim = c(-2.2, 2.2), ylim = c(-5, 2383)) +
    theme_minimal() +
